...
if Camera:
	...
	while True:
		...
		# BGRA変換
		BGRAFrame = cv2.cvtColor(frame, cv2.COLOR_BGR2BGRA)
		
		# HSV変換
		HSV = cv2.cvtColor(BGRAFrame, cv2.COLOR_BGR2HSV)
		
		# マスク生成
		Mask = cv2.inRange(HSV, (50, 0, 0), (80, 255, 255))
		...
		# 膨張処理画像と収縮処理画像の差分取得
		imgOfGPU = cp.max(ProcessingImg, axis=(2, 3)) - cp.min(ProcessingImg, axis=(2, 3))
		...
		# マクス削除
		# マスクの黒にあたる部分に48行目で取得した緑領域を代入、白にあたる部分に51∼61で作成した特徴量領域を代入
		img = cv2.bitwise_not(img, cv2.bitwise_not(BGRAFrame), mask=Mask)
